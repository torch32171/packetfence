...
For the VLAN Pool instead of defining a Vlan ID you can set a vlaue like that : 20..23,27..30 , it mean that the vlan returned by PacketFence can be (inclusive)  20 to 23 and 27 to 30. There are two algorithm, one based on a has of the username (default one) and the other one based on a round robin (last registered device +1).
