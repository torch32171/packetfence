...
For the VLAN pool instead of defining a Vlan identifier you can set a value like that : 20..23,27..30 , it mean that the VLAN returned by PacketFence can be (inclusive)  20 to 23 and 27 to 30. There are two algorithms, one based on a hash of the username (default one) and the other one based on a round-robin (last registered device +1).
